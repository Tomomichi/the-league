...
import { UserContext, DrawerContext } from '../lib/contexts.js';
import { firebase } from '../lib/firebase.js'
import { useSnackbar } from 'react-simple-snackbar'


export default function Drawer({opened}){
  ...
  return (
    ...
    <div className={`transform shadow top-0 right-0 w-2/3 sm:w-1/6 fixed h-full overflow-auto ease-in-out transition-all duration-300 z-50 ${drawerOpened ? 'translate-x-screen' : 'translate-x-full'}`}>
      <nav className="bg-white w-full h-full absolute right-0 top-0 z-50">
        ...
        <ul>
          <li className="p-4 cursor-pointer flex items-center hover:opacity-75" onClick={createLeague}>
            <svg className={`mr-1 w-5 h-5 fill-current`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          { user &&
            <li className="p-4 cursor-pointer hover:opacity-75 flex items-center" onClick={logout}>
              <svg className={`mr-1 w-5 h-5 fill-current`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
